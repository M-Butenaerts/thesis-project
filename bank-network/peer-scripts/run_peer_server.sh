...
chmod +x /usr/local/bin/peer && peer version
peer node start


# echo "===================== installing chaincode... ====================="
# peer lifecycle chaincode install /etc/hyperledger/chaincode/tar/bank_cc.tgz
...
# # export FPC_PATH=/opt/fabric-private-chaincode
# # export SGX_MODE=SIM                
# # export SGX_BUILD=DEBUG            
# # export PATH=$FPC_PATH/fabric/bin:$PATH

# CHAINCODE_VERSION="1.0"
# CHAINCODE_SEQUENCE="1"
# CHAINCODE_PACKAGE="/opt/fabric-private-chaincode/samples/chaincode/bank_cc/packages/bank_cc.tgz"

# # alias peer="/etc/hyperledger/peer-scripts/peer/wrapped-peer.sh" # replace peer with FPC peer
...
# #   peer lifecycle chaincode initEnclave \
...
# # SAVE PACKAGE ID
# PKGID_ERCC=$(peer lifecycle chaincode queryinstalled | awk -F'[ ,:]+' '/Label: ercc/{print $7}')

# peer lifecycle chaincode approveformyorg \
#   -o orderer.example.com:7050 --ordererTLSHostnameOverride orderer.example.com \
#   --tls --cafile "$ORDERER_CA" \
#   -C "$CHANNEL_NAME" -n ercc -v 1.0 --sequence 1 --package-id "$PKGID_ERCC"
