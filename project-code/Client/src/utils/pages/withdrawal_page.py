...
def submit_f(window, acc, ss, amount): 
    ...
    res = c.withdrawal(acc, signature, pk, date, amount, message["seq"])

    try:
        status = res.get("status")
        if status == "withdrawal made.":
            from utils.pages.main_page import main_page
            main_page(window, acc, ss)
...
def withdrawal_page(window, acc, ss):
    set_up_title(window)

    amount_label = tk.Label(window, text="account number:", bg=QUATERNARY_COLOR, fg=TERTIARY_COLOR, font=(FONT, LABEL_SIZE))
